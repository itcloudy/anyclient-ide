...
function defaultOptionRenderer<T>(v: { data: IDataOption<T>; isCurrent: boolean }) {
  return (
    <React.Fragment>
      {v.data.iconClass ? (
        <div className={classNames(v.data.iconClass, styles['select-option-icon'])}></div>
      ) : undefined}
      {v.data.label}
    </React.Fragment>
  );
...
function defaultGroupTitleRenderer<T>({ group, index }: { group: IDataOptionGroup<T>; index: number }) {
  return (
    <div key={'header_' + index} className={styles['select-group-header']}>
      {group.iconClass ? <div className={classNames(group.iconClass, styles['select-option-icon'])}></div> : undefined}
    ...
    return (
      <div
        ...
        className={classNames({
          [styles['select-option-select']]: value === element.props.value,
        })}
    ...
    return (
      <React.Fragment>
        {CustomSC ? (
          ...
          <React.Fragment>
            {selected.iconClass ? (
              <span className={classNames(selected.iconClass, styles['select-option-icon'])}></span>
            ) : undefined}
            <span className={styles['select-option']}>{selected.label}</span>
          </React.Fragment>
        )}
        {showDownIcon ? <Icon iconClass={getIcon('down')} /> : null}
      </React.Fragment>
    );
  };
      ...
      {open &&
        ...
        (isDataOptions(options) || isDataOptionGroups(options) ? (
          ...
          <div className={optionsContainerClasses} style={{ maxHeight: `${maxHeight}px` }} ref={overlayRef}>
            ...
            <div className={styles['select-overlay']} onClick={toggleOpen}></div>
          </div>
        ))}
    </div>
  );
...
function Description({ text }: { text: string }) {
  return (
    <>
      ...
      <div className={styles['option-description']}>{text}</div>
    </>
  );
}
