...
export class HiveDialect extends AbstractDefaultSqlDialect {
  ...
  showTables(connectInfo: IConnectInfo): string {
    // @formatter:off
    // @formatter:on
    return `SHOW TABLES IN ${connectInfo.db}`;
  }
  ...
  showViews(connectInfo: IConnectInfo): string {
    return `SHOW VIEWS  IN ${connectInfo.db}`;
  }
  ...
  createDb(param: IDbDetail): string[] {
    return [`create database ${param.schema}`];
  }
